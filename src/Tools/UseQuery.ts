...
export const loadProductsByDate = () => {
  const dispatch = useDispatch();
  const {
    loading: loadingProductsByDate,
    data: dataProductsByDate,
    error: errorProductsByDate,
  } = useQuery(GET_PRODUCTS_BY_DATE, {
    onCompleted: (dataProductsByDate) => {
      dispatch(setProductsByDate(dataProductsByDate.getProductsByDate));
    },
  });
}